...
class App():
    ...
    def update_clock(self):
        ...
        f=open("data4.txt")
        for j in range(App.N1):
            line1=f.readline()
        for j in range(App.N2):
            line2=f.readline()
        for j in range(App.N3):
            line3=f.readline()
        for j in range(App.N4):
            line4=f.readline()
        for j in range(App.N5):
            line5=f.readline()  
...
while len(finish) > 0:
    ...
    with open("data4.txt","a+") as f:                                                   #设置文件对象
        ...
        for i in finish:                                                                 #对于双层列表中的数据
            i = str(i).replace('[','进程优先级').strip('号').replace(']','').replace(',','剩余时间',1).replace(',','序号')+'\n' 
            f.write(i)  
    if time % 5 == 0:
